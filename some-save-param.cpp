...
void save_wb(Layer *layer, string weight_fn, string bias_fn) {
  int k_size = layer->kernel_h*layer->kernel_w;
  ofstream weight_file(weight_fn.c_str());
  // weight_file.open(weight_fn);
  for (int i = 0; i < layer->kernel_count; i++){
    for (int k = 0; k < k_size; k++){
      weight_file << layer->kernel[i].W[k] << endl;
    }
  }
  weight_file.close();

  ofstream bias_file(bias_fn.c_str());
  // bias_file.open(bias_fn);
  for (int i = 0; i < layer->map_count; i++){
    bias_file << layer->map[i].b << endl;
  }
  bias_file.close();
}
...
void save_param() {
  ...
  save_wb(&c1_conv_layer, c1_w_name, c1_b_name);   // C1
  save_wb(&s2_pooling_layer, s2_w_name, s2_b_name);// S2
  save_wb(&c3_conv_layer, c3_w_name, c3_b_name);   // C3
  save_wb(&s4_pooling_layer, s4_w_name, s4_b_name);// S4
  save_wb(&c5_conv_layer, c5_w_name, c5_b_name);   // C5
  save_wb(&output_layer, o_w_name, o_b_name);    // Out
}
