...
void update_params(Layer *layer, double learning_rate, int flag, string weight_fn, string bias_fn) {
    ...
    if(flag == 1){
    	...
    	for (int i = 0; i < layer->kernel_count; i++){
    		for (int k = 0; k < size; k++){
    			weight_file << layer->kernel[i].W[k];
    			weight_file << "\n";
    		}
    	}
    	weight_file.close();
		...
		ofstream bias_file;
		bias_file.open(bias_fn);
    	for (int i = 0; i < layer->map_count; i++){
    		bias_file << layer->map[i].b;
    		bias_file << "\n";
    	}
    	bias_file.close();
...
void update_weights(double learning_rate, int flag) {

	string c1_w_name = "c1w.txt";
	string c1_b_name = "c1b.txt";

	string s2_w_name = "s2w.txt";
	string s2_b_name = "s2b.txt";

	string c3_w_name = "c3w.txt";
	string c3_b_name = "c3b.txt";

	string s4_w_name = "s4w.txt";
	string s4_b_name = "s4b.txt";

	string c5_w_name = "c5w.txt";
	string c5_b_name = "c5b.txt";

	string o_w_name = "ow.txt";
	string o_b_name = "ob.txt";



    update_params(&c1_conv_layer, learning_rate, flag, c1_w_name, c1_b_name);   // C1
    update_params(&s2_pooling_layer, learning_rate, flag, s2_w_name, s2_b_name);// S2
    update_params(&c3_conv_layer, learning_rate, flag, c3_w_name, c3_b_name);   // C3
    update_params(&s4_pooling_layer, learning_rate, flag, s4_w_name, s4_b_name);// S4
    update_params(&c5_conv_layer, learning_rate, flag, c5_w_name, c5_b_name);   // C5
    update_params(&output_layer, learning_rate, flag, o_w_name, o_b_name);    // Out
}
...
//        cout << "i == " << i << endl;
        ...
        // 更新权值
        // +++++++++++++++++++++++++++++++++++++++++++++++++++++
        // set a flag at here to see if it is the last training
        int flag = 0;
        if(i == batch_count-1){
        	flag = 1;
        }

        update_weights(learning_rate, flag);
        // +++++++++++++++++++++++++++++++++++++++++++++++++++++
...
int main() {
    ...
    const char* train_sample_path = "/Users/yaoyuan/Desktop/CLeNet/train-images.idx3-ubyte";
    const char* train_label_path = "/Users/yaoyuan/Desktop/CLeNet/train-labels.idx1-ubyte";
