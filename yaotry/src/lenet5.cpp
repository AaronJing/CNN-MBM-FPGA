...
{
    double *data;//涔嬫墍浠ョ敤double鏄负浜嗗悗闈㈣绠楃粨鏋滄洿绮剧‘
    double *label;
...
const int batch_size = 10;//灏嗚缁冩暟鎹泦鍧囧垎鐨勪唤鏁�
const int classes_count = 10;//杈撳嚭鐨刲abel绉嶇被鏈�0,1,2,3... 鍗佺
const int width  = 32;
const int height = 32;
const int train_sample_count = 60000;//璁粌闆嗘牱鏈暟
const int test_sample_count  = 10000;//娴嬭瘯闆嗘牱鏈暟
...
void load_mnist_data(Sample * sample, const char * file_name){
    ...
    //鏂囦欢瀛樺偍鏍煎紡涓哄ぇ绔紝Intel CPU鏋舵瀯瀛樺偍涓哄皬绔紝鎵�浠ラ渶瑕佸皢瀛楄妭搴忓弽杞�
...
//                cout << i << "  "<< j << "---" << (double)temp/255 << endl;
                sample[k].data[(i + zero_padding) * width + j + zero_padding] = (double)temp/255 * (normalize_max - normalize_min) + normalize_min;//鎶婄伆搴﹀綊涓�鍖栧埌[0, 1]
            }
        }
    }
    fclose(fp);
...
void load_mnist_label(Sample * sample, const char * file_name){
    ...
    if(fp == NULL) {
        ...
        printf("load mnist label failed.");
        return;
    }
    int magic_number = 0;
    int sample_number = 0;
...
//        fread(&temp, 1, 1, fp);
//        sample[k].label[(int)temp] = 1;//todo 姝ｆ牱鏈琹abel璁炬垚1锛岃礋鏍锋湰label璁炬垚0
...
//鍒濆鍖朙ayer
//layer: 闇�瑕佽缁冪殑灞�
//prevlayer_map_count: 涓婁竴灞俵ayer鐨刴ap鏁�
//map_count: 鏈眰鐨刴ap鏁�
void init_layer(Layer *layer, int prevlayer_map_count, int map_count, int kernel_w, int kernel_h, int map_w, int map_h, bool is_pooling) {
    ...
    layer->kernel_count = prevlayer_map_count * map_count;//闇�瑕佽缁冪殑kernel鏁扮洰
...
void conv_fprop(Layer *prev_layer, Layer *layer, bool *pconnection) {
    ...
    for (int i = 0; i < layer->map_count; i++) {
        ...
        for (int k = 0; k < size; k++) {
            layer->map[i].data[k] = activation_func::tan_h(layer->map_common[k] + layer->map[i].b);//閲囩敤鍙屾洸姝ｅ垏鍑芥暟鏉ヨ〃绀烘縺娲诲嚱鏁�
        }
    }
}

//pooling灞傚墠鍚戜紶鎾� 鍙栨渶澶у��
...
void fully_connected_bprop(Layer *layer, Layer *prev_layer) {
    ...
    for (int i = 0; i < prev_layer->map_count; i++) {
        ...
        for (int j = 0; j < layer->map_count; j++) {
            prev_layer->map[i].error[0] += layer->map[j].error[0] * layer->kernel[i*layer->map_count + j].W[0];//鍏ㄨ繛鎺ラ殣钘忓眰鑺傜偣璇樊椤�
        }
        prev_layer->map[i].error[0] *= activation_func::dtan_h(prev_layer->map[i].data[0]);//鍏ㄨ繛鎺ラ殣钘忓眰鑺傜偣璇樊椤�
    }
    ...
    for (int k = 0; k < layer->map_count; k++) {
        ...
        for (int i = 0; i < map_h; i++) {
            for (int j = 0; j < map_w; j++) {
                ...
                prev_layer->map[k].error[row*upmap_w + col] = layer->map[k].error[i*map_w + j] * activation_func::dtan_h(max_value);//todo 杩欓噷闇�瑕佷箻浠an_h鐨勫鏁板嚱鏁板悧
            }
        }
    }

    //Pooling灞傚彧闇�瑕佸皢璇樊椤逛紶閫掑埌涓婁竴灞傦紝娌℃湁姊害鐨勮绠�
}
...
void update_params(Layer *layer, double learning_rate) {
    const double lambda = 0.0;//姝ｅ垯椤癸紝鏆傛椂璁句负闆�
...
void train(Sample *train_sample, double learning_rate) {
    ...
    for (i = 0; i < train_sample->sample_count; i++) {
        //todo 杩欎釜闅忔満浜ゆ崲浣嶇疆鐨勫嚱鏁版満鍒跺叕骞虫�ц繕鏈夊緟鑰冭檻
...
int main() {
    ...
    const char* train_sample_path = "MINST/train-images.idx3-ubyte";
    const char* train_label_path = "MINST/train-labels.idx1-ubyte";
    ...
    const char* test_sample_path = "MINST/t10k-images.idx3-ubyte";
    const char* test_label_path = "MINST/t10k-labels.idx1-ubyte";
